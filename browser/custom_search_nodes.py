...
class CustomSearchNode(ABC):
    ...
    @classmethod
    def from_parameter_type_opt_and_value(
        cls, browser, parameter_name, operator, value
    ):
...
class DifficultySearchNode(CustomSearchNode):
    ...
    def filter_ids(self, ids: Sequence[ItemId]) -> Sequence[ItemId]:
        ...
        ids = self._retain_ids_where(
            ids,
            f"json_extract(json_extract(data, '$.cd'), '$.d') {self.operator} {difficulty}",
        )
...
class StabilitySearchNode(CustomSearchNode):
    ...
    def filter_ids(self, ids: Sequence[ItemId]) -> Sequence[ItemId]:
        ...
        ids = self._retain_ids_where(
            ids,
            f"json_extract(json_extract(data, '$.cd'), '$.s') {self.operator} {stability}",
        )
