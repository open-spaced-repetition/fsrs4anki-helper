...
def get_due_range(cid, stability, due, desired_retention, maximum_interval):
    ...
    min_ivl, max_ivl = get_fuzz_range(new_ivl, greater_than_last(card, new_ivl), maximum_interval)
    if (
        ...
        min_ivl, max_ivl = get_fuzz_range(current_ivl, greater_than_last(card, current_ivl), current_ivl)
